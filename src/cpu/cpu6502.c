#include "cpu6502.h"
#include "cpu6502_opcodes.h"

void cpu6502_reset(cpu6502_t *cpu) {
  // load program counter with address stored at 0xFFFC (low byte) and 0xFFFD (high byte)
  cpu->reg_PC=cpu->read(0xFFFC)+(cpu->read(0xFFFD)<<8);
  cpu->reg_SP=0xFD; // reset stack pointer
  SET_FLAGS(0x24); // set following status flags: UNUSED, INTERRUPT
}

#define SET_FLAGS(v) do{ \
    cpu->status_C=(v)&0x01; \
    cpu->status_Z=((v)>>1)&0x01; \
    cpu->status_I=((v)>>2)&0x01; \
    cpu->status_D=((v)>>3)&0x01; \
    cpu->status_B=((v)>>4)&0x01; \
    cpu->status_U=((v)>>5)&0x01; \
    cpu->status_V=((v)>>6)&0x01; \
    cpu->status_N=((v)>>7)&0x01; \
  } while(0)

#define RECALC_ZN(v) do{ \
    cpu->flag_Z=(v)?1:0; \
    cpu->flag_N=(v)&0x80?1:0; \
  } while(0)

#define GET_FLAGS() ((cpu->status_N<<7)|(cpu->status_V<<6)|(cpu->status_U<<5)| \
  (cpu->status_B<<4)|(cpu->status_D<<3)|(cpu->status_I<<2)|(cpu->status_Z<<1)| \
  cpu->status_C)

#define PUSH(v) do { \
    cpu->write(0x100|cpu->reg_SP, (v)&0xFF); \
    cpu->reg_SP--; \
    if (cpu->reg_SP<0) { \
      cpu->reg_SP=0xFF; \
    } \
  } while(0)

#define READ16(adr) (cpu->read(adr)|(cpu->read((adr)+1)<<8))

#define READ16BUG(adr) (cpu->read(adr)|(cpu->read(()(adr)&0xFF00)+()((adr)+1)&0xFF))<<8))

#define PAGE_DIFFERS(a,b) ((a)&0xFF00!=(b)&0xFF00)

#define BRANCH(c) do { \
    if (c) { \
      cycles_passed+=PAGE_DIFFERS(cpu->reg_PC, address)?2:1; \
      cpu->reg_PC=address; \
    } \
  } while(0)

#define COMPARE(a, b) do { \
    RECALC_ZN((a)-(b));
    cpu->flag_C= (a)>=(b)?1:0;
  } while(0)

int cpu6502_run(cpu6502_t *cpu, int cycles_to_run) {
  opcode_tbl_entry opcode;
  int cycles_passed; // cycles used in one iteration
  int address; // calculated address for memory interaction
  int temp_value; // temporary value used for calculation

  do {
    cycles_passed=0;

    // check if cpu is stalling
    if (cpu->stall_cycles) {
      cycles_passed=cpu->stall_cycles;
      cpu->stall_cycles=0;
    }

    // check for interrupts
    if (cpu->interrupt_pending) {
      PUSH(cpu->reg_PC>>8); // high byte of program counter to stack
      PUSH(cpu->reg_PC); // low byte
      PUSH(GET_FLAGS() | 0x10); // push status flags to stack
      if (cpu->interrupt_pending==INTERRUPT_NMI) {
        cpu->reg_PC=READ16(0xFFFA);
      }
      else {
        cpu->reg_PC=READ16(0xFFFE);
      }
      cpu->flags_I=1;
      cycles_passed+=7;
      cpu->interrupt_pending=INTERRUPT_NONE,
    }

    // read op code
    opcode=opcode_tbl[cpu->read(cpu->reg_PC++)];

    // handle address mode
    switch (opcode.address_mode) {
      case ADR_ABSOLUTE:
        address=READ16(cpu->reg_PC+1);
        break;
      case ADR_ABSOLUTE_X:
        address=READ16(cpu->reg_PC+1)+cpu->reg_X;
        page_crossed=PAGE_DIFFERS(address-cpu->reg_X, address);
        cycles_passed+=PAGE_DIFFERS(address-cpu->reg_X, address)?opcode.page_cross_cycles:0;
        break;
      case ADR_ABSOLUTE_Y:
        address=READ16(cpu->reg_PC+1)+cpu->reg_Y;
        cycles_passed+=PAGE_DIFFERS(address-cpu->reg_Y, address)?opcode.page_cross_cycles:0;
        break;
      case ADR_ACCUMULATOR:
        address=0;
        break;
      case ADR_IMMEDIATE:
        address=cpu.PC+1;
        break;
      case ADR_IMPLIED:
        address=0;
        break;
      case ADR_INDEXED_INDIRECT:
        address=READ16BUG(cpu->read(cpu->PC+1)+cpu->X)
        break;
      case ADR_INDIRECT:
        address=READ16BUG(READ16(cpu->reg_PC+1))
        break;
      case ADR_INDIRECT_INDEXED:
        address=READ16BUG(cpu->read(cpu->reg_PC+1))+cpu->reg_Y;
        cycles_passed+=PAGE_DIFFERS(address-cpu->reg_Y, address)?opcode.page_cross_cycles:0;
        break;
      case ADR_RELATIVE:
        address=cpu->read(cpu->reg_PC+1)
        if (address<0x80) {
          address+=cpu->reg_PC+2;
        }
        else {
          address+=cpu->reg_PX+2-0x100;
        }
        break;
      case ADR_ZERO_PAGE:
        address=cpu->read(cpu->reg_PC+1)
        break;
      case ADR_ZERO_PAGE_X:
        address=cpu->read(cpu->reg_PC+1)+cpu->reg_X;
        break;
      case ADR_ZERO_PAGE_Y:
        address=cpu->read(cpu->reg_PC+1)+cpu->reg_Y;
        break;
    }

    address&=0xFFFF; // mask 16 bit

    cpu->reg_PC+=opcode.bytes; // update program counter
    cycles_passed+=opcode.cycles; // update cycles for this opcode

    switch(opcode.operation) {
      case OP_ADC:
        cpu->reg_A=cpu->reg_A+cpu->read(address)+cpu->flag_C;
        cpu->flag_C=cpu->reg_A>0xFF?1:0;
        cpu->flag_V=cpu->reg_A>0x7F?1:0;
        RECALC_ZN(cpu->reg_A);
        cpu->reg_A&=0xFF;
        break;
      case OP_AND:
        cpu->reg_A=cpu->reg_A&cpu->read(address);
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_ASL:
        if (opcode.address_mode==ADR_ACCUMULATOR) {
          cpu->flag_C= cpu->reg_A&0x80?1:0;
          cpu->flag_A= (cpu->reg_A&0x7F)<<1;
          RECALC_ZN(cpu->reg_A);
        }
        else {
          temp_value=cpu->read(address);
          cpu->flag_C= temp_value&0x80?1:0;
          temp_value=(temp_value&0x7F)<<1;
          cpu->write(address, temp_value);
          RECALC_ZN(temp_value);
        }
        break;
      case OP_BCC:
        BRANCH(!cpu->flag_C);
        break;
      case OP_BCS:
        BRANCH(cpu->flag_C);
        break;
      case OP_BEQ:
        BRANCH(cpu->flag_Z);
        break;
      case OP_BIT:
        temp_value=cpu->read(address);
        cpu->flag_V=(temp_value&0x40)?1:0;
        cpu->flag_Z=(temp_value&cpu->reg_A)?1:0
        cpu->flag_N=temp_value?1:0;
        break;
      case OP_BMI:
        BRANCH(cpu->flag_N);
        break;
      case OP_BNE:
        BRANCH(!cpu->flag_Z);
        break;
      case OP_BPL:
        BRANCH(!cpu->flag_N);
        break;
      case OP_BRK:
        PUSH(cpu->reg_PC>>8); // high byte of program counter to stack
        PUSH(cpu->reg_PC); // low byte
        PUSH(GET_FLAGS() | 0x10); // push status flags to stack
        cpu->flag_I=1;
        cpu->reg_PC=READ16(0xFFFE);
        break;
      case OP_BVC:
        BRANCH(!cpu->flag_V);
        break;
      case OP_BVS:
        BRANCH(cpu->flag_V);
        break;
      case OP_CLC:
        cpu->flag_C=0;
        break;
      case OP_CLD:
        cpu->flag_D=0;
        break;
      case OP_CLI:
        cpu->flag_I=0;
        break;
      case OP_CLV:
        cpu->flag_V=0;
        break;
      case OP_CMP:
        COMPARE(cpu->reg_A, cpu->read(address));
        break;
      case OP_CPX:
        COMPARE(cpu->reg_X, cpu->read(address));
        break;
      case OP_CPY:
        COMPARE(cpu->reg_Y, cpu->read(address));
        break;
      case OP_DEC:
        temp_value=cpu->read(address)-1;
        if (temp_value<0) {
          temp_value=0xFF;
        }
        cpu->write(address, temp_value);
        RECALC_ZN(temp_value);
        break;
      case OP_DEX:
        cpu->reg_X--;
        if (cpu->reg_X<0) {
          cpu->reg_X=0xFF;
        }
        RECALC_ZN(cpu->reg_X);
        break;
      case OP_DEY:
        cpu->reg_Y--;
        if (cpu->reg_Y<0) {
          cpu->reg_Y=0xFF;
        }
        RECALC_ZN(cpu->reg_Y);
        break;
      case OP_EOR:
        cpu->reg_A=cpu->reg_A^cpu->read(address);
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_INC:
        temp_value=cpu->read(address)+1;
        if (temp_value>255) {
          temp_value=0x00;
        }
        cpu->write(address, temp_value);
        RECALC_ZN(temp_value);
        break;
      case OP_INX:
        cpu->reg_X++;
        if (cpu->reg_X>255) {
          cpu->reg_X=0x00;
        }
        RECALC_ZN(cpu->reg_X);
        break;
      case OP_INY:
        cpu->reg_Y++;
        if (cpu->reg_Y>255) {
          cpu->reg_Y=0x00;
        }
        RECALC_ZN(cpu->reg_Y);
        break;
      case OP_JMP:
        cpu->reg_PC=address;
        break;
      case OP_JSR:
        cpu->reg_PC--;
        PUSH(cpu->reg_PC>>8); // high byte of program counter to stack
        PUSH(cpu->reg_PC); // low byte
        cpu->reg_PC=address;
        break;
      case OP_LDA:
        cpu->reg_A=cpu->read(address);
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_LDX:
        cpu->reg_X=cpu->read(address);
        RECALC_ZN(cpu->reg_X);
        break;
      case OP_LDY:
        cpu->reg_Y=cpu->read(address);
        RECALC_ZN(cpu->reg_Y);
        break;
      case OP_LSR:
        if (opcode.address_mode==ADR_ACCUMULATOR) {
          cpu->flag_C= cpu->reg_A&0x01?1:0;
          cpu->flag_A= cpu->reg_A>>1;
          RECALC_ZN(cpu->reg_A);
        }
        else {
          temp_value=cpu->read(address);
          cpu->flag_C= temp_value&0x01?1:0;
          temp_value>>=1;
          cpu->write(address, temp_value);
          RECALC_ZN(temp_value);
        }
        break;
      case OP_NOP:
        break;
      case OP_ORA:
        cpu->reg_A=cpu->reg_A|cpu->read(address);
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_PHA:
        PUSH(cpu->reg_A);
        break;
      case OP_PHP:
        PUSH(GET_FLAGS()|0x10);
        break;
      case OP_PLA:
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        cpu->reg_A=cpu->read(0x100|cpu->reg_SP); 
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_PLP:
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        SET_FLAGS(cpu->read(0x100|cpu->reg_SP)&0xEF|0x20); 
        break;
      case OP_ROL:
        if (opcode.address_mode==ADR_ACCUMULATOR) {
          cpu->flag_A=((cpu->reg_A<<1)&0xFF)+(cpu->reg_A>>7);
          cpu->flag_C=cpu->reg_A&0x01?1:0;
          RECALC_ZN(cpu->reg_A);
        }
        else {
          temp_value=cpu->read(address);
          temp_value=((temp_value<<1)&0xFF)+(temp_value>>7);
          cpu->flag_C=temp_value&0x01?1:0;
          cpu->write(address, temp_value);
          RECALC_ZN(temp_value);
        }
        break;
      case OP_ROR:
        if (opcode.address_mode==ADR_ACCUMULATOR) {
          cpu->flag_A=(cpu->reg_A>>1)+((cpu->reg_A&0x01)<<7);
          cpu->flag_C=cpu->reg_A&0x80?1:0;
          RECALC_ZN(cpu->reg_A);
        }
        else {
          temp_value=cpu->read(address);
          temp_value=(temp_value>>1)+((temp_value&0x01)<<7);
          cpu->flag_C=temp_value&0x80?1:0;
          cpu->write(address, temp_value);
          RECALC_ZN(temp_value);
        }
        break;
      case OP_RTI:
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        SET_FLAGS(cpu->read(0x100|cpu->reg_SP)&0xEF|0x20); 
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        cpu->reg_PC=cpu->read(0x100|cpu->reg_SP); 
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        cpu->reg_PC+=cpu->read(0x100|cpu->reg_SP)<<8; 
        break;
      case OP_RTS:
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        cpu->reg_PC=cpu->read(0x100|cpu->reg_SP); 
        cpu->reg_SP=(cpu->reg_SP+1)&0xFF; 
        cpu->reg_PC+=cpu->read(0x100|cpu->reg_SP)<<8; 
        cpu->reg_PC+=1;
        break;
      case OP_SBC:
        cpu->reg_A=cpu->reg_A-cpu->read(address)-(1-cpu->flag_C);
        cpu->flag_C=cpu->reg_A>=0?1:0;
        cpu->flag_V=cpu->reg_A>127||cpu->reg_A<-128?1:0;
        RECALC_ZN(cpu->reg_A);
        cpu->reg_A&=0xFF;
        break;
      case OP_SEC:
        cpu->flag_C=1;
        break;
      case OP_SED:
        cpu->flag_D=1;
        break;
      case OP_SEI:
        cpu->flag_I=1;
        break;
      case OP_STA:
        cpu->write(address, cpu->reg_A);
        break;
      case OP_STX:
        cpu->write(address, cpu->reg_X);
        break;
      case OP_STY:
        cpu->write(address, cpu->reg_Y);
        break;
      case OP_TAX:
        cpu->reg_X=cpu->reg_A;
        RECALC_ZN(cpu->reg_X);
        break;
      case OP_TAY:
        cpu->reg_Y=cpu->reg_A;
        RECALC_ZN(cpu->reg_Y);
        break;
      case OP_TSX:
        cpu->reg_X=cpu->reg_SP;
        RECALC_ZN(cpu->reg_X);
        break;
      case OP_TXA:
        cpu->reg_A=cpu->reg_X;
        RECALC_ZN(cpu->reg_A);
        break;
      case OP_TXS:
        cpu->reg_SP=cpu->reg_X;
        break;
      case OP_TYA:
        cpu->reg_A=cpu->reg_Y;
        RECALC_ZN(cpu->reg_A);
        break;
      // undocumented opcodes
      case OP_AHX:
      case OP_ALR:
      case OP_ANC:
      case OP_ARR:
      case OP_AXS:
      case OP_DCP:
      case OP_ISC:
      case OP_KIL:
      case OP_LAS:
      case OP_LAX:
      case OP_RLA:
      case OP_RRA:
      case OP_SAX:
      case OP_SHX:
      case OP_SHY:
      case OP_SLO:
      case OP_SRE:
      case OP_TAS:
      case OP_XAA:
        break;
    }

    cycles_to_run-=cycles_passed;
  } while (cycle_to_run>0);

  return cycle_index;
}

void cpu6502_trigger_interrupt(cpu6502_t *cpu, cpu6502_interrupt_t interrupt) {
  // TODO: could it happen that NMI and IRQ occuring the same time?
  switch (interrupt) {
    case INTERRUPT_NMI:
      cpu->interrupt_pending=INTERRUPT_NMI;
      break;
    case INTERRUPT_IRQ:
      if (cpu->status_I) {
        cpu->interrupt_pending=INTERRUPT_NMI;
      }
      break;
  }
}
